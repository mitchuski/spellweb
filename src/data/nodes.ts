...
export const NODES: SpellwebNode[] = [
  ...
  { id: "zk-tale-32", type: "act", label: "ZK Tale 32: The Flock 🌗", domain: "swordsman", layer: "narrative", spellbook: "zero_knowledge", desc: "The Flock. Blade 61 (111101, tale convention; seated V47 under the v10.4 lattice lock) — Sovereignty without projection: Protection + Memory + Connection + Computation + Value lit, Delegation alone dark. Moon Phase 🌗 Last Quarter (stratum 5). V(π,t) terms: [C, A_h(τ), Value]. Vertex ⟨1,0,1,1,1,1⟩. Flock — the binary-field SNARK for batches of Boolean computation (Rothblum · Bünz · Wang; Succinct / Espresso, 2026-06-25): GF(2^k) towers with ring-switching, Ligerito hash-based commitment, no trusted setup; standard hashes (SHA-256, Keccak, BLAKE3) at under 250× native cost — 82,100 BLAKE3 compressions/s on one core, 660,000+ on ten, enough hashing for ≈4,000 TPS under leanVM; the prover Ethereum needs to aggregate hash-based signatures after the quantum transition. Counting: 23 + 38 = 61 (Folding Path + Lethe), 61/38 ≈ φ; complement bnot(61) = 2, pure Delegation, unnamed — a pair opens. Second frontier tale of the Quest; 16 blades named, 48 wait. Technical Bridge transferred to the DTG ZK Book (primer P5, stack flock).", emoji: "🕊️", proverb: "The cheapest proof is of the hash you already have; keep the ledger you had, and fly in the field where bits are already numbers." },
  ...
  { id: "con-7thcapital", type: "concept", label: "7th Capital", domain: "first_person", layer: "knowledge", desc: "Behavioral sovereignty as personal wealth — your data, digital fabric, behavioral patterns, preferences, knowledge, social graphs. Its character is soil, not treasure: land-based rentier capital that yields rent by position, leased never conveyed, eroding on the adversary's background-information clock (C55 re-typed 2026-07-17; bridge C97)." },
  ...
  { id: "con-dragon-vertex", type: "concept", label: "Dragon Vertex", domain: "first_person", layer: "knowledge", desc: "The 7th Capital. The single vertex at the peak of the Boolean lattice. All six dimensions activated. Maximum sovereignty. Not a place to travel to—the coordinate you project from. Held ground, not hoarded treasure: the vertex pays rent for as long as it is held (soil ruling, C55/C97)." },
  ...
  },
  {
    ...
    domain: 'shared',
    layer: 'knowledge',
    desc: 'Time stealing entropy back from the surveillance economy. Reclaimed as soil: ground taken back pays rent to its holder — only reputation compounds.',
    emoji: '⏱️→🔙',
  },
  ...
  { id: "skill-dual-agent-harness", type: "skill", label: "Dual-Agent Harness", emoji: "♻️", domain: "shared", layer: "knowledge", desc: "The shareable soulbis ⚔️ ⊥ soulbae 🧙 skeleton (~/dual-agent-harness · rebuilt 2026-07-09, supersedes the v1.0 extraction): six-phase loop Measure → Propose → Hold-apart → Assay → Critic → Chronicle, seven seats, the six PVM trusts in TRUSTS.md, conform.mjs proving (⚔️⊥⿻⊥🧙)😊 = neg ⊕ bnot → succ on Z/64Z. The Gap ⿻ derives held-out witnesses by hashing the proposal together with a run secret the proposer never sees (non-collusion invariant I(Y_S;Y_M|X)=0); verdicts are VALIDATED / MIRAGE / BLOCKED; only the keystone folds, only the First Person publishes. Fifteen instances stand in HARNESS_PATHS.md — the origin fleet, the operator's work WITH the harness kept apart from the system (accessions to sixteen, one withdrawn; #1 shor_mage · ecdsa.fail, #2 myterms · the browser-extension pair, #3 the V6 rehydration pipeline, #4 privacy_pools_v2_mage · first mechanically fitted, #5 lexon_pvm · the grammar workshop, #14 the DTG verification registry · first external run accepted, #16 uor_kappa_mage · the constitution inherited whole). Its own field-guide spar folded 730 → 573 → 526 → 472 words across three audited folds, the last two keystone-census 32/32. Built for trust-graph construction: a gated result seals as a κ-addressed holon; a derived edge proposes, only a VRC signature mints (GRAPH.md).", version: "2.1" },
  ...
  { id: "conj-c55", type: "concept", label: "C55 · Privacy as the Seventh Capital", domain: "shared", layer: "knowledge", desc: "Privacy is the seventh kind of capital, foundationally; its character is land-based RENTIER capital (data as soil): it yields rent by position, not value by aggregation; its worth is the appropriation share the consent interface sets; its exposure erodes on the adversary's background-information clock (C82); and it is leased, never conveyed — structural inalienability by non-reconstruction (C97) is the ownability mechanism. Compounding is scoped to reputational fertility only, never to raw records. Character re-typed 2026-07-17 (First-Person soil ruling, L149). Tome III Act III.9. Resonance Mana (🔭) is the 7th Capital in motion.", conjectureId: "C55", conjectureStatus: "provisional", conjectureConfidence: 0.7 },
  ...
  { id: "conj-c82", type: "concept", label: "C82 · The Moving Ceiling R(t)", domain: "shared", layer: "knowledge", desc: "Adversary informational capability grows against fixed archives: the linkage corpus and side priors accumulate along calendar time, shrinking H(X|B_t) while nothing is added to the archive and no action of the subject is involved; R_inf(t) drifts upward on a schedule and every static reconstruction guarantee has a finite shelf life t*. Frontier-model releases enter only informationally (better extraction of linkage from existing corpora), never as compute against the information-theoretic guarantee, which is compute-saturated. Re-typed 2026-07-17 (L149); erosion form proven conditional in WP-07 Def 3.9 + Cor 5.4b; the conjectural content is the RATE. Active (65% · Run 1).", conjectureId: "C82", conjectureStatus: "active", conjectureConfidence: 0.65, conjectureRegister: "core" },
  ...
  { id: "concept-dtg-credentials", type: "concept", label: "the Decentralized Trust Graph (DTG)", emoji: "🕸️", domain: "shared", layer: "knowledge", desc: "The relationship layer issued native on Archon did:cid / VC 2.0: VRC (relationship) · VMC (membership) · VIC (invitation) · VPC (persona) · VEC (endorsement) · VWC (witness) · RCard (contact card). Policy lives in a two-faced TRQP trust registry (authorizes issuers outward · grades a Witness's autonomy inward), not the credential. Specified upstream as the ToIP DTG Credentials Core Specification: six DTGCredential types on four DID roles (R/M/C/P), with two ZK constructions — pairwise (prove the edge, hide the R-DIDs) and community-anchored (edge + membership under one C-DID, personhood carried forward); the ZK layer itself is deferred to the DTG ZKP Task Force.", proverb: "Thin credential, fat registry." },
  ...
  // ═════════════════════════════════════════════════════════════
  // THE FIELD GUIDE WEAVE · convergence merge (2026-08-17) — the trust
  // predicate walked into a street. Max's ARWorld handoff (2026-08-15) met
  // by a counter-spec built as evidence: eight suites, 124 properties, all
  // green as the field repo certified them. Source: ~/field_guide_privacymage
  // (built on the field farm, served from the embassy Pis, merged home as
  // fieldguide.localhost — the guide federation's 13th site). Numbers are the
  // field repo's; this graph adds no digit. Tome IX Act 10 PROPOSED, unbound.
  // ═════════════════════════════════════════════════════════════
  { id: "gateway-field-guide", type: "gateway", label: "the Hitchhikers Field Guide (ARWorld)", emoji: "🧭", domain: "shared", layer: "knowledge", desc: "Max's walking AR city, built on OASIS — multiplayer means: you Witness a place (a rite, not a pin-tap), the fact lands in Knowledge, Promise decides what a peer may see, Trust decides whose Knowledge reaches your map. A quiet overlay, never live avatars, never a score. As a client of the graphs it is a conformant PAIRWISE deployment of the DTG cred-spec — a pseudonym graph, not a personhood graph: every privacy property survives, no Sybil-resistance is claimed.", proverb: "Discovery is a rite, not a pin." },
  { id: "doc-field-guide-counter-spec", type: "document", label: "The Field Guide Counter-Spec", emoji: "🧭", domain: "shared", layer: "knowledge", desc: "COUNTER-SPEC-meet-and-overlay.md + REFLECTION-MAP.md — the reply to the ARWorld handoff, built as evidence rather than argued as prose: eight suites, 124 properties (meet-overlay 54/54 · 22-code rejection register 8/8 · zero-shared-code Python consumer 15/15 byte-exact · lab bridge importing DTG runtimes 07+01 10/10 · the reflection map itself checked 6/6). The correspondence table is executed, not maintained — a row whose property dies goes red. Found the hole in the lab: its formation runtime gates every edge on personhood, modelling only the community-anchored construction; the pairwise case had never been exercised.", version: "2026-08-16", proverb: "The fastest way to disagree with any of this is a failing test." },
  { id: "concept-meet-rite", type: "concept", label: "the Meet Rite", emoji: "🤝", domain: "shared", layer: "knowledge", desc: "How a 1-hop trust edge is made in a street, between two phones: the lab's formation predicate (bilateral consent, no self-edge, fresh face per counterparty, the Gap ⿻ — one phone recomputes what the other claims) plus exactly four gates the physical world adds, asserted as four: an offer expires in two minutes, a nonce is single-use (a photographed QR is already spent), fifty metres, and confirm-is-the-opener bookkeeping. The Gap survived the substrate change unaltered — the strongest evidence it is structural.", proverb: "A meet is a moment, not a credential in a wallet." },
  { id: "concept-audience-granted", type: "concept", label: "audience is granted, not computed", emoji: "🎚️", domain: "shared", layer: "knowledge", desc: "The overlay's refusal of transitive trust: 'does trust reach' is a true question about a graph, but the map asks 'who may see', and answering the second with the first is how a trust score gets built by accident. Two hops is not half of link — it is nothing. Reachability is not visibility; no threshold required.", proverb: "Two hops is not half of link. It is nothing." },
  { id: "concept-pairwise-peer-reference", type: "concept", label: "the pairwise peer reference", emoji: "🎭", domain: "shared", layer: "knowledge", desc: "The cred-spec's R-DID rule (a fresh, unique reference per counterparty) enforced at the overlay: two viewers' projections of one pin share NO identifier field, so two people who both met the same walker cannot join their views. The shipped game returned a raw avatar guid + global display name on every peer pin; the fix — a face chosen per edge at meet time — costs one text field.", proverb: "A face per edge, chosen at the meet." },
  { id: "concept-publication-consent", type: "concept", label: "a wiki fork is not an audience", emoji: "📣", domain: "shared", layer: "knowledge", desc: "The disclosure ladder answers 'how much do I show THIS counterparty'; a federated page has no counterparty — it is readable by everyone and propagates by design. So 'public' the tier and 'public' the page are false friends in the worst direction. Publication is its own consent, defaulting off, and WORLD is deliberately not on the ladder, so it cannot be reached by widening.", proverb: "I will show you where I stood. I will not tell the world I stood anywhere." },
  { id: "concept-erosion-rate", type: "concept", label: "erosion is a rate, not a cliff", emoji: "⏳", domain: "shared", layer: "knowledge", desc: "Assurance decays continuously: a meet from four years ago is not a meet from Tuesday, and overlay recency (the '90 days?' question) is the same X6 finding arrived at independently from a map-rendering problem. The two clocks are asymmetric — your own trail never erodes. Not yet applied to the edge itself in the shipped model; named honestly as the open piece.", proverb: "A meet from four years ago is not a meet from Tuesday." },
  { id: "concept-rounding-rule", type: "concept", label: "rounding is not a detail", emoji: "📐", domain: "shared", layer: "knowledge", desc: "The public tier coarsens location to ~100m; JavaScript rounds midpoints up, Python and C# round to even — the same pin lands 110m apart, opposite sides of a street, and both implementations pass their own tests. A coarsening rule that does not name its rounding mode is not a rule. Generalises to every numeric field in a canonical encoding — found within an hour by a zero-shared-code consumer.", proverb: "Two correct implementations, one pin, opposite sides of the street." },
  { id: "chron-field-guide-convergence", type: "chronicle", label: "Chronicle: The Field Guide Comes Home (2026-08-17)", emoji: "🧭", domain: "shared", layer: "knowledge", desc: "Binds the arc: 08-15 the predicate walks to Islington (the counter-spec built, the lab bridged, the pairwise correction), 08-16 the checklist answers back (a colleague's unrun acceptance list catches the leak the leak-test missed; both strands served to the team from the embassy Pis), 08-17 the convergence merge (fieldguide.localhost seated as the federation's 13th site; fork-with-lineage, manifest-first). Twice in two days the correcting instrument was another hand's document made executable." },

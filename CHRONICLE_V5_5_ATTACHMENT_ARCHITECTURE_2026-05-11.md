...
|---|---|---|---|
| `vertex-v4` | V4 | Mnemosyne (pure Memory) | Mnemosyne 📿 |
| `vertex-v8` | V8 | Iris (pure Connection) | Iris 🌈 |
| `vertex-v16` | V16 | Logos / Pure Computation | Pythia 🔥 |
| `vertex-v38` | V38 | Lethe · the Dark Substrate | Lethae 🌘 |

### New cast nodes (7)

| id | Cast | Vertex | Primary persona(s) | Kind | Divergence | Status |
|---|---|---|---|---|---|---|
| `cast-lethae` | Lethae 🌘 | V38 | `moonkeeper` | B_cross_shop | **mage_register** | anticipated |
| `cast-mnemosyne` | Mnemosyne 📿 | V4 | `theia` | A_workshop | none | anticipated |
| `cast-iris` | Iris 🌈 | V8 | `herald`, `ambassador` | A_workshop | none | anticipated |
| `cast-pythia` | Pythia 🔥 | V16 | `algebraist`, `pedagogue` | A_workshop | none | anticipated |
...
| `src/data/nodes.ts` | Appended a new V5.5 section before END OF NODES: 4 new vertex nodes (V4 · V8 · V16 · V38) and 7 new cast nodes (Lethae + 6 anticipated). All carry the new V5.5 fields. |
...
- The 14 existing cast nodes (Soulbis, Soulbae, Pallia, Memora, Vulcana, etc.) were *not* updated with the V5.5 fields. Their existing schema works as-is; adding `attachmentKind` / `divergence` / `abstractPersonaIds` to them is a follow-up backfill pass.
